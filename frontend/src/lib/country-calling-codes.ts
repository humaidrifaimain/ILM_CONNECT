...
export const DEFAULT_COUNTRY_ISO = 'LK';
...
export const COUNTRY_CALLING_CODES: CountryCallingCode[] = [
  ...
  { iso2: 'CD', name: 'Congo DR', dialCode: '+243' },
...
const countryByIso = new Map(COUNTRY_CALLING_CODES.map((country) => [country.iso2, country]));

export function getCountryCallingCode(iso2: string | null | undefined) {
  return countryByIso.get((iso2 || DEFAULT_COUNTRY_ISO).toUpperCase()) || countryByIso.get(DEFAULT_COUNTRY_ISO)!;
}
...
export function formatInternationalPhone(rawPhone: string, country: CountryCallingCode) {
